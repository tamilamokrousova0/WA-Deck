...
async function updateHubDashboard() {
  ...
  for (const account of accounts) {
    ...
    status.className = 'hub-acct-status';
    let isOnline = false;
    if (!account.frozen) {
      const wv = state.webviews.get(account.id);
      isOnline = !!(wv && wv.dataset && wv.dataset.waReady === '1');
    }
    status.classList.add(account.frozen ? 'frozen' : (isOnline ? 'on' : 'off'));
    ...
    let statusText = '';
    if (account.frozen) statusText = '❄ заморожен';
    else if (isOnline) statusText = 'онлайн';
    else {
      const wv = state.webviews.get(account.id);
      statusText = wv ? 'загрузка…' : 'не загружен';
    }
